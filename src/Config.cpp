...
namespace tld {
    ...
    int Config::configure(FindMe *findMe) {

        // findMe
        findMe->tld->trackerEnabled = m_settings.m_trackerEnabled;



        findMe->threshold = m_settings.m_threshold;



        // if (m_settings.m_initialBoundingBox.size() > 0) {
        //     findMe->initialBB = new int[4];

        //     for (int i = 0; i < 4; i++) {
        //         findMe->initialBB[i] = m_settings.m_initialBoundingBox[i];
        //     }
        // }

        DetectorCascade *detectorCascade = findMe->tld->detectorCascade;
